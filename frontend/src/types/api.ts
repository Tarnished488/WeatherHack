...
export interface LlmAdviceResponse {
  ...
  risk_score: number
  risk_level: RiskLevel
  // The server answers with per-role LLM advice when the model succeeds,
  // and falls back to the rule-engine template strings when it does not.
  advice: Record<AudienceRole, RoleAdvice> | Record<AudienceRole, string[]>
}
